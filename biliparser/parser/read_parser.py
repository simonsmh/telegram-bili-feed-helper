...
async def __relink(client: httpx.AsyncClient, img):
...
async def parse_read(client: httpx.AsyncClient, url: str):
    ...
    # 1.获取缓存
    try:
        cache_base = RedisCache().get(f"read:page:{f.read_id}")
    except Exception as e:
        logger.exception(f"拉取文章页面缓存错误: {e}")
        cache_base = None
    # 2.拉取文章
    if cache_base:
        logger.info(f"拉取文章页面缓存: {f.read_id}")
        cv_content = orjson.loads(cache_base)  # type: ignore
    else:
        try:
            r = await client.get(f.url)
        except Exception as e:
            raise ParserException(f"文章页面获取错误:{f.read_id}", url, e)
            # 3.解析文章
        cv_init = re.search(r"window\.__INITIAL_STATE__=(.*?);\(function\(\)", r.text)
        if not cv_init:
            raise ParserException(f"文章页面内容获取错误:{f.read_id}", url, cv_init)
    ...
    # 1.获取缓存
    try:
        cache_graphurl = RedisCache().get(f"read:graphurl:{f.read_id}")
    except Exception as e:
        logger.exception(f"拉取文章链接缓存错误: {e}")
        cache_graphurl = None
    # 2.拉取文章
    if cache_graphurl:
        logger.info(f"拉取文章链接缓存: {f.read_id}")
        ...
        try:
            ...
            if not isinstance(article, Tag):
                raise ParserException("文章内容解析错误", url, cv_content)
            imgs = article.find_all("img")
            task = list(__relink(client, img) for img in imgs)  ## data-src -> src
